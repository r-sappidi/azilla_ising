"""EXPERIMENTAL shared-fetch integration; requires new differential validation.
...
from .shared_fetch import SharedFetchPipeline as CoreIterationPipeline
...
def run_full_cores(model, records, initialization, *, iterations=1, arithmetic_fixture=None):
    ...
    for groups,base in ((canonical.h0,0),(canonical.h1,total_h0),(canonical.cross,total_h0+g.node_count)):
        for endpoint,jobs in enumerate(groups):
            for work in jobs:
                ...
                queues[base+endpoint].append(DmaCommand(a,b,a,b))
    ...
    controller.driver.jobs_per_iteration *= 2
    ...
    return ExactEventResult(
        ...
        weight_block_reads=len(records)*iterations,logical_weight_blocks_stored=len(records),
        remote_state_packets=len(directed.state_publications)*iterations,core_weight_buffers=1,
        core_pipeline_contract=dict(model='experimental_shared_fetch_full_iteration_v1', fetch_policy='single_fetch_two_unicast_v1',
