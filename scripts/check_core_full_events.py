...
from azilla_cycle_model.core_full_events import run_full_cores
...
def main():
    ...
    if not total or int(total[2])!=12:ap.error('requires unique three-block two-iteration fixture')
    ...
    report=dict(matched=all(checks.values()),checks=checks,scope='full wrapper; 256 spins; two iterations; specified engines/config',
        ...
        wrapper_sha256=hashlib.sha256((ROOT/'model/azilla_cycle_model/core_full_events.py').read_bytes()).hexdigest())
