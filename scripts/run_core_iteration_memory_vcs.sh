...
output=${OUTPUT_ROOT:-results/paper_validation_20260907/core_iteration_memory_v1}
mesh=${USE_MESH:-1}
engines=${ENGINES:-1}
jobs=${JOBS_PER_SOURCE:-2}
memory_config=${RAMULATOR_CONFIG:-tb/ramulator_128x32.yaml}
build="$repo/build/core_iteration_memory_vcs_mesh${mesh}_e${engines}_j${jobs}"
...
/usr/bin/time -v -o "$output/build.time" \
  ...
  -timescale=1ns/1ps -top core_iteration_memory_vcs_tb \
  -assert svaext -assert disable_cover \
  -pvalue+core_iteration_memory_vcs_tb.USE_MESH="$mesh" \
  -pvalue+core_iteration_memory_vcs_tb.ENGINES="$engines" \
  -pvalue+core_iteration_memory_vcs_tb.JOBS_PER_SOURCE="$jobs" \
  ...
  tb/core_iteration_memory_vcs_tb.sv tb/ramulator_dpi.cpp \
...
for stall in 0 7 13; do
  ...
  rg -q 'PASS CORE_ITERATION_MEMORY' "$output/stall${stall}.log"
