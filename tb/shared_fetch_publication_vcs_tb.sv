...
module shared_fetch_publication_vcs_tb #(parameter bit USE_MESH=1, parameter int ENGINES=1,
  ...
  logic [SOURCES-1:0][ENGINES-1:0] raw_cv, raw_cr, raw_wv, raw_wr;
  logic [SOURCES-1:0][ENGINES-1:0] replay_idle;
  logic [SOURCES-1:0][ENGINES-1:0][GW-1:0] raw_ca,raw_cb;
  logic [SOURCES-1:0][ENGINES-1:0][DATA_W-1:0] raw_wd;
  ...
  int external_lock[4],external_choice[4];
  logic [4:0] external_beat[4];
  always @(posedge clk) begin
    if(rst) for(int h=0;h<4;h++) begin external_lock[h]=-1;external_beat[h]=0;end
    else if(DOUBLE_BUFFER) for(int c=0;c<CORES;c++) begin
      if(cwv[c] && cwr[c] && active_source[c]>=1) begin
        external_lock[c/2] <= external_beat[c/2]==31 ? -1 :
          (active_source[c]==2 ? ENGINES : active_engine[c]);
        external_beat[c/2] <= external_beat[c/2]+1'b1;
      end
    end
  end
  ...
  for(genvar s=0;s<SOURCES;s++) begin: memories
    ...
    ramulator_node_frontend #(.SYSTEM_ID(SYS),.MVM_COUNT(COUNT),
      ...
      .node_cmd_valid_o(raw_cv[s][COUNT-1:0]),.node_cmd_ready_i(raw_cr[s][COUNT-1:0]),
      .node_state_a_index_o(),.node_state_b_index_o(),
      .node_block_a_id_o(raw_ca[s][COUNT-1:0]),.node_block_b_id_o(raw_cb[s][COUNT-1:0]),
      .node_weight_valid_o(raw_wv[s][COUNT-1:0]),.node_weight_ready_i(raw_wr[s][COUNT-1:0]),
      .node_weight_data_o(raw_wd[s][COUNT-1:0]),.idle_o(raw_idle[s]));
    assign idle[s] = raw_idle[s] && (&replay_idle[s][COUNT-1:0]);
    for(genvar e=0;e<COUNT;e++) begin: replay_lanes
      if(DOUBLE_BUFFER) begin: double_slots
        shared_fetch_replay_double #(.BLOCK_W(GW)) replay (
          .clk,.rst,.in_cmd_valid(raw_cv[s][e]),.in_cmd_ready(raw_cr[s][e]),
          .in_a(raw_ca[s][e]),.in_b(raw_cb[s][e]),
          .in_weight_valid(raw_wv[s][e]),.in_weight_ready(raw_wr[s][e]),.in_weight_data(raw_wd[s][e]),
          .out_cmd_valid(cv[s][e]),.out_cmd_ready(cr[s][e]),.out_a(ca[s][e]),.out_b(cb[s][e]),
          .out_weight_valid(wv[s][e]),.out_weight_ready(wr[s][e]),.out_weight_data(wd[s][e]),
          .idle_o(replay_idle[s][e]));
      end else begin: single_slot
      assign replay_idle[s][e]=raw_cr[s][e];
      shared_fetch_replay #(.BLOCK_W(GW)) replay (
        .clk,.rst,.in_cmd_valid(raw_cv[s][e]),.in_cmd_ready(raw_cr[s][e]),
        .in_a(raw_ca[s][e]),.in_b(raw_cb[s][e]),
        .in_weight_valid(raw_wv[s][e]),.in_weight_ready(raw_wr[s][e]),.in_weight_data(raw_wd[s][e]),
        .out_cmd_valid(cv[s][e]),.out_cmd_ready(cr[s][e]),.out_a(ca[s][e]),.out_b(cb[s][e]),
  ...
  always_comb begin
    ...
    for(int h=0;h<4;h++) external_choice[h]=external_lock[h];
    if(DOUBLE_BUFFER) begin
      for(int e=0;e<ENGINES;e++) begin
        int dst;
        dst=int'(ca[1][e]);
        if(wv[1][e] && active_source[dst]==1 && active_engine[dst]==e && external_choice[dst/2]<0)
          external_choice[dst/2]=e;
      end
      for(int n=0;n<2;n++) begin
        int dst;
        dst=int'(nob[n][LOCAL]);
        if(nov[n][LOCAL] && notype[n][LOCAL]==3 && external_choice[dst/2]<0)
          external_choice[dst/2]=ENGINES;
      end
    end
    ...
    if(USE_MESH && !pub_network && !completion_network) begin
      ...
      for(int n=0;n<2;n++) begin
        ...
        if(nov[n][LOCAL]) begin
          ...
          if(active_source[dst]==2 && cwr[dst] &&
             (!DOUBLE_BUFFER || external_choice[dst/2]==ENGINES)) begin
    ...
    for(int s=0;s<SOURCES;s++) for(int e=0;e<ENGINES;e++) begin
      int dst;
      dst=int'(ca[s][e]);
      if((!USE_MESH||s!=2) && wv[s][e] && active_source[dst]==s && active_engine[dst]==e &&
         (DOUBLE_BUFFER ? (!cwv[dst] && (s==0 || (!used[dst/2] && external_choice[dst/2]==e))) :
                          (!used[dst/2] && !used_source[s]))) begin
        cwv[dst]=1;cwd[dst]=wd[s][e];wr[s][e]=cwr[dst];
        if(!DOUBLE_BUFFER || s!=0) used[dst/2]=1;
        used_source[s]=1;
      end
    end
  end
  ...
  always @(negedge clk) begin
    ...
    if(!rst && dispatch_enable) for(int s=0;s<SOURCES;s++) begin
      ...
      allocated=issued[s];
      for(int e=0;e<ENGINES;e++) begin
        sv[s][e]=(e<source_engines(s) && allocated<(JOBS_PER_SOURCE/2) && sr[s][e]);
        sa[s][e]=GW'(0);
        sb[s][e]=GW'(other);
        if(sv[s][e]) allocated++;
      end
